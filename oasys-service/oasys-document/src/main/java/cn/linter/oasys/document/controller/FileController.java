...
public class FileController {
    ...
    @GetMapping("files")
    public Result<Page<File>> listFile(@RequestParam(defaultValue = "1") @ApiParam("页号") int pageNumber,
    ...
    public Result<File> createFolder(@RequestBody @ApiParam("文件夹实例") File file) {
        return Result.of(ResultStatus.SUCCESS, fileService.createFolder(file));
    }

    @ApiOperation("上传文件")
    @PostMapping("file")
    public Result<File> uploadFile(@ApiParam("二进制文件") MultipartFile multipartFile, @ApiParam("文件实例") File file) throws Exception {
        return Result.of(ResultStatus.SUCCESS, fileService.uploadFile(multipartFile, file));
    }
    ...
    public ResultStatus deleteFile(@PathVariable("id") @ApiParam("文件ID") Long id) throws IOException, InvalidResponseException, InvalidKeyException,
